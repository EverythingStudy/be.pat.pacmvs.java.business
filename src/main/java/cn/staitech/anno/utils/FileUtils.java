...
//        }
    ...
    public static Object updateGeojson(Features marking, String geojsonUrl) throws Exception {
        // 读取原始json文件并进行操作和输出
        File jsonFile = new File(geojsonUrl);
        //通过getStr方法获取json文件的内容
        if (!jsonFile.exists()) {
        ...
        for (Object feature : features) {
            JSONObject featureObject = (JSONObject) feature;
            JSONObject properties = featureObject.getJSONObject("properties");
            Long id = properties.getLong("marking_id");
            System.out.println(marking.getProperties().getMarking_id());
            if (Objects.equals(id, marking.getProperties().getMarking_id())) {
                if (marking.getProperties().getLabel_color() != null) {
                    properties.put("label_color", marking.getProperties().getLabel_color());
                }
                if (marking.getGeometry() != null) {
                    featureObject.put("geometry", marking.getGeometry());
                }
                if (marking.getProperties().getArea() != null) {
                    properties.put("area", marking.getProperties().getArea());
                }
                if (marking.getProperties().getPerimeter() != null) {
                    properties.put("perimeter", marking.getProperties().getPerimeter());
                }
                if (marking.getProperties().getDescription() != null) {
                    properties.put("description", marking.getProperties().getDescription());
                }
                if (marking.getProperties().getCategory_id() != null) {
                    properties.put("category_id", marking.getProperties().getCategory_id());
                    properties.put("label_name", marking.getProperties().getLabel_name());
                    properties.put("label_color", marking.getProperties().getLabel_color());
                }
                if (marking.getProperties().getMeasure_full_name() != null) {
                    properties.put("measure_full_name", marking.getProperties().getMeasure_full_name());
                }

                if (marking.getProperties().getMeasure_name() != null) {
                    properties.put("measure_name", marking.getProperties().getMeasure_name());
                }
                if (marking.getProperties().getMeasure_number() != null) {
                    properties.put("measure_number", marking.getProperties().getMeasure_number());
                }
                res = feature;
            }
        }
        if (ObjectUtil.isEmpty(res)) {
            throw new Exception("未发现标注信息");
        }
        // 将文件写入json中
        FileUtils.createJsonFile(parse, geojsonUrl);
        return res;
    }


    public static Object updatePointCount(Features marking, String geojsonUrl) throws Exception {
        // 读取原始json文件并进行操作和输出
        File jsonFile = new File(geojsonUrl);
        //通过getStr方法获取json文件的内容
        if (!jsonFile.exists()) {
        ...
        for (Object feature : features) {
            JSONObject featureObject = (JSONObject) feature;
            JSONObject properties = featureObject.getJSONObject("properties");
            Long id = properties.getLong("category_id");
            String locationType = properties.getString("location_type");
            if (Objects.equals(id, marking.getProperties().getCategory_id()) && Objects.equals(locationType, marking.getProperties().getLocation_type())) {
                properties.put("point_count", marking.getProperties().getPoint_count());
                res = feature;
            }
        }
        // 将文件写入json中
